...
public class Stream4 {
    public static void main(String[] args) {
        ...
        peek : 각 요소를 순회하면서 Consumer를 작업을 실행지연 (Stream에 영향 끼치지 않음 - 지연평가)
        ...

        최종 연산
        - 실제로 계산을 실행하고, 리턴값을 갖거나, Side Effect 를 발생시킴
        - 최종 연산시 스트림을 모두 소모해서, 더 이상 스트림 사용 불가
        forEach : 각 요소를 순회하면서 Consumer를 작업을 실행
        collect : 스트림 요소를 리스트, 셋, 멥, 등으로 반환
        ...
        min :  Comparator에 맞는 최소값 반환
        max : Comparator에 맞는 최대값 반환
        ...
         */
    }
}
