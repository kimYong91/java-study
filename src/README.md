...
## 보조 스트림
    1. 문자열 변환 스트림 : InputStreamReader, OutputStreamWriter
      - 바이트 스트림 -> 문자열 스트림, 바이트의 데이터의 특정 문자 인코딩을 텍스트로 읽거나 쓸 수 있음
    2. 성능향상 스트림 : BufferedInputStream, BufferedOutputStream
      - 데이터를 처리할 때 내부적으로 버퍼를 사용해서 데이터를 임시 저장하여 입출력 효율을 높임
    3. 기분타입 스트림 : DataInputStream, DataOutputStream
    ...
    5. 객체 직렬화 스트림 : ObjectInputStream, ObjectOutputStream
      - 객체를 직렬화(객체 -> 바이트 스트림) 혹 역직렬화(바이트 스트림 -> 객체)

## 직렬화 (Serialization)
    - 객체의 상태를 바이트 스크림으로 변환
    - 네크워크 등을 통하여 다른 시스템에 전달할 때 사용
...
### IP 주소 (Internet Protocol Address)
    ...
    - IP 주소 버전
        1. IPv4
            ...
            - 32비크 주소 체계 약 43억개의 고유 주소를 가짐
            - 예시 : 192.168.0.1
            - (윈도우에서 IP주소 확인 방법) -> 명령프롬프트 -> ipconfig 입력
        2. IPv6
    ...
    - 게이트웨이
        - 서로 다른 네트워크 또는 서브 네크워크 간의 통신을 가능하게 하는 네트워크 장치
        - 일반적으로 라우터가 게이트웨이 역할을 수행
        - 라우터(공유기) LAN과 WAN간의 게이트웨이 역할로 다른 네크워크의 출입구
    ...
    - DNS(Domain Name System)
        ...
        - 도메인 네임 서버 : 도메인 이름 - IP를 매치시키는 역할을 하는 서버
            ...
            - 권한 네임 서버 : naver.com / google.com ....
                - 해당 도메인과 관련된 모든 DNS 코드 정보를 가지고 있음
    ...
    - 포트 번호
        ...
        - 분류
            1. 잘 알려진 포트 : 0 ~ 1023, IANA에서 할당
                ...
                - FTP (파일 전송) : 21
                - SSH (보안 접속) : 22
            2. 등록 포트 : 1024 ~ 49151, 특정 서비스에 할당될 수 있음 (사용자 정의, 변경 될 수 있음)
                - MySql : 3306
                - Oracle : 1521
            3. 동적 포트 : 49152 ~ 65535, OS 에서 동적으로 할당, 개인적으로 일시적 통신에 사용
                -[공식 등록 포트 번호 일람](https://www.iana.org/assignments
                                        /service-names-port-numbers/service-names-port-numbers.xhtml)

## URL (Uniform Resource Locator)
    - URI (Uniform Resource Indicator)
        - 자원을 식별하는데 사용되는 고유의 식별자
    ...
    - URI의 구조
        ...
        - schme://user:password@host:port.path?query#fragment
        - schme : 리소스에 접근하기 위한 특정한 프로토콜(규약) http, https, ftp등
        - host : 리소스가 호스팅되는 서버의 도메인 이름 또는 IP 주소
        - port : 리소스에 접근하기 위한 서버의 포트 번호 (http: 80, https: 443 기본포트 생략)
        - query : 리소스에 전단되는 추가 정보 'key = value' 형태로 포함
...
## JDBC (Java DataBase Connectivity)
    ...
    - API가 데이커베이스에 독립적이어서 다양한 종류의 DBMS에 연결할 수 있는 방법 제공
    - 구성 클래스
...
### JDBC 드라이브 설치
    ...
    - MySQ의 경우 Installer에서 Connector/J 를 설치하거나
...
### DAO 패턴
    ...
    - 데이터베이스 연결, 퉈리 실행 등 DB 접근 코드로부터 비즈니스 로직을 분리
    - 구성 요소
