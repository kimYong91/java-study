...
public class Stream12 {
    ...
    // 목적 : 작업 처리 시간을 줄이는 것
    // 동시성 : Concurrency
    // 여러개의 스레드가 하나의 코어에서 번갈아가면서 실행 > 사람 눈에는 병렬과 같이 동시에 진행하는 것으로 보이지만 실제로는 순차적으로 하나씩 처리.
    // 병렬성 : Parallelism
    // 멀티 코어에서 각각의 작업을 병렬로 실행하는 것

    // 자바 병렬 스트림에서는
    // Fork / Join Framework
    // Fork : 데이터의 스트림을 쪼개어서 멀티 코어에 나누어 줌 -> 병렬로 처리  * 작업량이 작으면 병렬이 더 느릴수 있다.
    ...
    public static void main(String[] args) {
        ...
        int nCount = 100_000_000;       // 바꿔가며 비교
