...
public class Quiz4 {
    public static void main(String[] args) {
        DataProcessor<String> text = new StringDataProcessor();
        DataProcessor<Integer> number = new NumberDataProcessor();
        String s = text.processData("hello generics");
        Integer integer = number.processData(100);
...
제네릭 인터페이스를 이용한 데이터 처리 시스템
    ...
    processData 메소드를 정의합니다. 이 메소드는 T 타입의 데이터를 매개변수로 받고, 처리된 데이터의 결과를 T 타입으로 반환합니다.
    ...
    main 메서드 구현:
    DataProcessor<String> 타입 객체와 DataProcessor<Integer> 타입 객체를 생성하고 각각 “hello generics”, 100을 입력값으로 processData 메서드를 호출합니다.
