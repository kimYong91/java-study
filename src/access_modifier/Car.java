...
public class Car {
    ...
    public int getSpeed() {
        if (isStop == false) {      // 멈춰있지 않을 때 : 시동이 걸렸을 때 * (!isStop)와 같음
            return speed;
        } else {
            return 0;               // 시동이 꺼진 상태를 설정 가능
        }

    }
    // Setter method : 안전하게 필드 값을 설정할 때 사용된다.
    public void setSpeed(int speed) {
        //세터 메서드에서 제약사항을 추가할 수 있다.
        if (speed >= 0 && speed <= 120) {
            this.speed = speed;
        } else {
            System.out.println("0~120까지의 속도만 성절 가능합니다.");
        }
        this.speed = speed;
    }
