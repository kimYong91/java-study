...
public class Object1 {
    public static void main(String[] args) throws InterruptedException {
        ...
        // 런타임 도중에 다른 클래스의 메서드나, 필드, 생성자 등에 접근해서 객체를 생성하거나, 메서드 호출하거나 필드 변경하거나 할 수 있음 => Reflection

        object.notify();        // 객체를 기다리는 스레드 중 하나를 때움
        object.notifyAll();     // 객체를 기다리는 모든 스레드를 깨움
        object.wait();          // 현제 스레드를 일시 중단하고 깨울때까지 기다림
        object.wait(1000);  // 스레드 중단하고 n밀리초 기다림
        object.wait(100,100);   // ㅅ스레드 중단하고 n밀리초, m나노초 기다림

        // object.clone();         // 객체의 복제본을 생성하는 메서드
        // object.finalize();     // 더이상 사용하지 않는 메서드 -> Garbage Collector 에 의해 객체가 삭제되기 전에 호출되는 메서드
    }
}
