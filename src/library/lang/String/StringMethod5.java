...
public class StringMethod5 {
    public static void main(String[] args) {
        ...
        // concat 메서드가 사용될 때 마다 문자열이 새로 생성된다.(문자열 = 불변객체)
        // 내부적으로 더 효율적인 방법 => StringBuilder, StringBuffer

        // 변경이 가능한 문자열을 활용할 수 있음.
        StringBuilder stringBuilder;        // 다중 스레드 환경일때 사용
        StringBuffer stringBuffer;          // 단일 스레드 환경일때 사용 => 성능이 더 좋음

        StringBuilder sb1 = new StringBuilder();

        // 메서드 체이닝 기법, append()메서드는 문자열 뒤에 내용을 추가하고 stringBuilder 타입을 return 함. 최종 문자열을 반환하고 싶을때는 toString()
        String result = sb1.append(la1)
                .append(la2)            // '.'은 가독성이 떨어짐으로 한 줄씩 만듬
                .append(" ")
                .append(la3)
                .toString();
